...
function! Tex_CompileMultipleTimes()
	...
	while needToRerun == 1 && runCount < 5
		...
		" If .nlo file changed, then run makeindex to generate the new .nls
		" file and remember to rerun latex.
		if runCount == 0 && glob(nloFileName) != '' && nlolinesBefore != nlolinesAfter
			echomsg "Running makeindex nomenclature..."
			let temp_mp = &mp | let &mp = Tex_GetVarValue('Tex_MakeIndexFlavor')
			exec 'silent! make '.mainFileName_root.'.nlo -s nomencl.ist -o '.mainFileName_root.'.nls'
