...
def getSectionLabels(lineinfo, 
    ...
    # Problem with original version: it did not work well with e.g.
    # \chapter[short]{Long}. The most natural way to solve it would be to
    # include something like (\[.*\])?, but split includes the match of the
    # parenthsis in the list, which breaks the whole concept. Therefore each
    # element is include "separately"
